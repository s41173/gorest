...
func Register(c *gin.Context) {
	...
	// Validasi city
	if !city.CheckCity(config.DB, form.CCity) {
		c.JSON(400, gin.H{"error": "City Not Found"})
		return
	}

	// Validasi chapter
	if !chapter.CheckChapter(config.DB, form.CChapter) {
		c.JSON(400, gin.H{"error": "Chapter Not Found"})
		return
	}

	// Validasi nomor telepon
	if customer.CheckUserPhone(config.DB, form.TPhone1) {
		c.JSON(400, gin.H{"error": "Phone registered"})
		return
	}

	// Validasi email
	if customer.CheckUser(config.DB, form.TEmail) {
	...
	latesID, err := customer.GetLatestID(config.DB)
	if err != nil {
	...
	logdata := ""
	device := ""
	err = log.AddLog(config.DB, int(latesID), &logdata, &device)
	if err != nil {
...
func Update_user(c *gin.Context) {
	...
	if !city.CheckCity(config.DB, requestData.City) {
		c.JSON(400, gin.H{"error": "City Not Found"})
		return
	}
	...
	if err := config.DB.First(&customer, claims.UserID).Error; err != nil {
	...
	chaptercode := chapter.GetChapterCode(config.DB, int16(customer.ClubID))
...
func Index_city(c *gin.Context) {
	...
	city := models.City{}
	res, err := city.GetAllCities(config.DB)
	if err == nil {
